...
The `claude-legal` **marketplace** — a Claude Code / Cowork marketplace of court-document plugins organized one plugin per state, plus a shared data-only plugin for federal law. It ships **eight plugins total — seven state plugins + one shared data-only plugin**:
