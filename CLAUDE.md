...
The `claude-legal` **marketplace** — a Claude Code / Cowork marketplace of court-document plugins organized one plugin per state. It ships **three** plugins today (two complete, one scaffolded):
...
- **`ca-court-docs`** — California (**scaffolded only**; CRC 2.100-2.119 formatting, LASC + SFSC + populous-counties roll-up, Rosenthal Act / CCP consumer-debt bundle planned). All 21 SKILL.md files and reference READMEs are lint-clean stubs with TODO markers — substantive content authoring is the next phase. Do not present `ca-court-docs` as production-ready until the stubs are filled in.
...
## Skills index — California (`ca-court-docs`, 21 skills — **scaffolded stubs, not authored**)

Every entry below is a lint-clean stub generated by `scaffold-state-plugin`. The "Role" column describes the intended content; the actual SKILL.md bodies are placeholders awaiting substantive content. Same 21-skill shape as the WA / OR plugins.

| Skill | Role (intended; stub today) |
|---|---|
...
**Stubs only at this stage.** Mirrors the WA/OR corpora structure. Each subdirectory contains a `README.md` placeholder noting that the corpus is to be populated by a future pull script:

- **`court-rules/`** — California court rules (CRC, local Superior Court rules) from courts.ca.gov.
- **`federal-debt-laws/`** — FDCPA, FCRA, TILA, ECOA, Reg F/V/Z/B (federal law; identical content to the WA / OR corpora — re-populate from `pull_federal_debt_laws.py`).
- **`ucc-model/`** — Model UCC Articles 1, 2, 3, 9.
- **`ca-statutes-debt/`** — California statute chapters most relevant to civil practice (CCP §§ 337, 337a (4-year SOL on written contract), §§ 412.20 et seq. (service), §§ 1010 et seq. (notices and motions); Cal. Civ. Code §§ 1788 et seq. (Rosenthal Act); Cal. Fin. Code §§ 100000 et seq. (CDCLA); Cal. Bus. & Prof. Code §§ 17200 et seq. (UCL)).

No California pull scripts exist yet; they are tracked as a TODO alongside the Oregon pull scripts.
