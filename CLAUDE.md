...
The `claude-legal` **marketplace** — a Claude Code / Cowork marketplace of court-document plugins organized one plugin per state, plus a shared federal-law plugin (canonical federal corpora + a nationwide consumer credit-report-rights skills layer). It ships **eleven plugins total — ten state plugins + one shared federal-law plugin**:
