...
- **`claude-legal-immigration-laws`** — Shared, venue-independent reference plugin for U.S. immigration law (the federal counterpart to `claude-legal-federal-laws`, built for immigration practice). **As of v0.3.0 it ships both reference corpora AND an 11-skill, matter-neutral, document-producing self-help layer** (it began references-only, like the federal plugin pre-v0.4.0; the skills layer is described in its own index below). Snapshots the canonical **rules** verbatim as Markdown via the pullers: the **INA** (`scripts/pull_ina.py` — 8 U.S.C. Chapter 12 from uscode.house.gov USLM XML, one file per subchapter I–V, with an INA-section ↔ 8 U.S.C.-section crosswalk in the corpus README), the immigration **regulations** (`scripts/pull_immigration_cfr.py` — curated 8 CFR DHS chapter I + EOIR chapter V (8 CFR 1001–1337, the immigration courts + Board of Immigration Appeals) + State Department 22 CFR visa/passport/exchange parts, from the eCFR versioner API), and the **Foreign Affairs Manual** (`scripts/pull_fam.py` — **verbatim** 9 FAM 100/302/402/502/504 + 8 FAM 300 + 7 FAM 000/1400; fam.state.gov serves an incomplete TLS chain (omits the Sectigo intermediate) and Python's `ssl` won't AIA-chase it, so the puller opens an unverified handshake purely to read the leaf's AIA "CA Issuers" URL, fetches the intermediate, and adds it to a verify bundle — after which all content fetches are fully TLS-verified; it then crawls the site's JSON TOC API `/api/Tree/GetTreeByVolumeId` and the per-section `/FAM/<vol>/<id>.html` windows-1252 pages; falls back to pointer stubs only when the network is unreachable, with the `_file_is_stub` guard preserving committed verbatim text). Immigration-court procedure also gets a **`court-rules/`** corpus — the binding **8 CFR Part 1003 / 1240 / 1208** regulations (verbatim, in `immigration-regulations/`) plus pointer stubs for the **Immigration Court Practice Manual** and the **BIA Practice Manual** (EOIR renders these client-side and Akamai-gates them, so `scripts/pull_eoir_manuals.py` writes canonical-URL stubs with the chapter enumeration + a cross-reference to the binding 8 CFR rules). **Case law is wired on-demand, not snapshotted** (immigration case law is too large/fast-moving to mirror): `references/legal-data-apis.md` + `references/online-sources.md` index the federal **circuit courts** (petitions for review of removal orders under INA § 242 / 8 U.S.C. § 1252, via CourtListener REST + the connected CourtListener MCP, with per-circuit `court` ids), the **Board of Immigration Appeals** precedent decisions (I&N Dec., via the EOIR Virtual Law Library), and the USCIS **Administrative Appeals Office (AAO)** decisions (website/PDF — no JSON API). Marketplace metadata bumped to 0.24.0 when this plugin was added.
...
- **`court-rules/`** — Colorado court rules. **72 Chief Justice Directives (CJDs)** including the foundational **CJD 11-01 Statewide Electronic Filing Standards** (effective Jan 1, 2026) are auto-pulled verbatim from `coloradojudicial.gov` by `scripts/pull_co_court_rules.py`. The rule sets themselves — **C.R.C.P., CRE, C.A.R., Colorado Rules of Professional Conduct** — are edicts of the Colorado Supreme Court, so the **rule text is public domain**; but the only conveniently published copies are the *annotated compilations* West (Colorado Court Rules) and LexisNexis copyright (editorial notes + formatting), and Colorado posts no free government mirror of the bare consolidated text, so the corpus carries pointer stubs for those sets and the streamlined Chapter 18 county-court / Chapter 25 small-claims rules. Total corpus: ~1.3 MB across 78 MD files.
- **`federal-debt-laws/`** *(symlink)* — points into `claude-legal-federal-laws/references/federal-debt-laws/`.
- **`federal-bankruptcy/`** *(symlink)* — points into `claude-legal-federal-laws/references/federal-bankruptcy/`.
- **`ucc-model/`** *(symlink)* — points into `claude-legal-federal-laws/references/ucc-model/`.
...
- **`federal-bankruptcy/`** *(symlink)* — points into `claude-legal-federal-laws/references/federal-bankruptcy/`.
- **`ucc-model/`** *(symlink)* — points into `claude-legal-federal-laws/references/ucc-model/`.
- **`tn-statutes-debt/`** — **25 chapters of the Tenn. Code Ann. verbatim (~2.6 MB / 1,496 sections)** covering the civil-practice + consumer-debt + family-law + landlord-tenant + tort + commercial surface (Titles 15, 16, 20, 24, 26, 27, 28, 29, 36, 37, 47, 62, 66). Pulled by `scripts/pull_tn_statutes.py` from the Justia mirror at `law.justia.com/codes/tennessee/`. The Tennessee Code's **statutory text is public domain** — a government edict, so no one copyrights the words of the law (Banks v. Manchester; Georgia v. Public.Resource.Org); what LexisNexis copyrights is the *Annotated* compilation it publishes as the official code under state contract, and Tennessee posts no clean structured free copy of the bare text (its free hottopics mirror is a JS container, not flat HTML). The puller copies only the section text — not the annotations — from the structured free mirrors Justia/FindLaw. Both sit behind **Cloudflare bot-fight mode** that fingerprints the TLS handshake — stdlib urllib gets a 403 even from a residential IP. **As of `_manifest.json` v0.2.0** the puller uses **`curl_cffi` with Chrome TLS impersonation** and walks **Chapter → Part → Section** for chapters subdivided into Parts; run from a residential or VPN-routed IP, it produces verbatim Markdown for all 25 chapters. Falls back to **well-formed pointer stubs** carrying the canonical Justia + LexisNexis URLs when fetch fails. The `_file_is_stub` regression guard preserves any verbatim content already committed. Directory name retains the legacy `debt` slug for path stability.
...
- **`foreign-affairs-manual/`** — the **FAM**, mirrored **verbatim** (~3.6 MB across 6 files): 9 FAM 100 overview + 302 ineligibilities + 402 NIV classifications + 502/504 IV categories/processing; 8 FAM 300 (the only substantive public nationality chapter — the rest of 8 FAM is SBU/redacted); 7 FAM 000 consular-protection overview + 1400 documentation of major life events / CRBA. fam.state.gov serves an incomplete TLS chain (omits the Sectigo *Public Server Authentication CA OV R36* intermediate) and Python's `ssl` doesn't AIA-chase — this is the **server's own** misconfiguration, not an egress proxy or destination bot-gate (so unlike the NY-rules / TN-statutes Cloudflare gates, `curl_cffi` is irrelevant here). `scripts/pull_fam.py` fixes it from any machine: it opens an unverified handshake purely to read the leaf's AIA "CA Issuers" URL, downloads the intermediate, and adds it to a certifi-roots-+-intermediate verify bundle (all content fetches then fully verified), then crawls the JSON TOC API `/api/Tree/GetTreeByVolumeId?Id=<vol>` and the per-section `/FAM/<vol>/<id>.html` windows-1252 pages. Pointer stubs are written only when the network is unreachable; the `_file_is_stub` guard preserves committed verbatim text. FAM is agency guidance — persuasive, not binding — so cites are always paired with the controlling INA / CFR provision.
- **`court-rules/`** — the procedural "court rules" of the **EOIR immigration courts**. The **binding** rules are already verbatim in `immigration-regulations/` (8 CFR Part 1003 / 1240 / 1208); this dir adds the two EOIR **practice manuals** — the **Immigration Court Practice Manual (ICPM)** and the **BIA Practice Manual** — as **pointer stubs** (canonical landing + per-chapter URLs + scope + a cross-reference to the binding 8 CFR rules). EOIR renders the manuals client-side (JavaScript) and Akamai-gates the chapter URLs, so a stdlib client can't mirror verbatim text; `scripts/pull_eoir_manuals.py` writes the stubs (and would write verbatim from a headless / un-gated run, with the `_file_is_stub` guard preserving it). The `eoir-immigration-courts` skill is the consumer of this corpus.
...
Versioning: a routine corpus refresh bumps the relevant `_manifest.json` (`last_pulled`, `release_point` / `as_of`, `mode`) but not `plugin.json`; editing a skill body or its triggers bumps that `SKILL.md`'s `version:`; adding/removing a corpus dir or a skill, or changing which sources are covered, is a `plugin.json` version bump. (The skills layer arrived in `plugin.json` 0.3.0.)
