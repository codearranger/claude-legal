...
- **`jury-instructions/`** — **pointer-stub index (NOT verbatim)** of the Washington Pattern Jury Instructions: **WPI** (Civil, the plugin's focus) + **WPIC** (Criminal). Unlike the RCW and court-rules corpora (public-domain edicts snapshotted verbatim), WPI/WPIC are **copyrighted by Thomson Reuters/West** and offered free to the public only by agreement (legal-practice/research use; no commercial re-use or redistribution), and `govt.westlaw.com` is bot-gated — so this corpus carries a topic-number index + canonical pointers (AOC landing page + the West free-access civil URL) + access posture, **not** the instruction text. There is **no `pull_*.py`** for it by design. Follows the same "honest stub for copyrighted/paywalled content" discipline as the CO annotated rules, the NY Tanbook, and the EOIR practice manuals.
