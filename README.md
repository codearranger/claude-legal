...
| `ny-court-docs` | 31 NY consolidated-laws targets via NYSENATE API (CPLR + GOB + GBS + RPAPL + RPL + UCC + DRL + EPT + GCN + BNK; verbatim with `NYSENATE_API_KEY`, stubs otherwise) | 12 rule sets via `pull_ny_court_rules.py` (22 NYCRR Parts 130/202/202.70/208/210/212/214/1200 + Tanbook + NYC Civil Court + Nassau DC + Suffolk DC; verbatim or pointer stubs depending on Cloudflare reachability) | shared |
...
│   └── ny-court-docs/                # New York (27 skills — 5 flagship Supreme Court venues + 2 dedicated Long Island District Courts + 2 subject bundles)
...
A GitHub Actions workflow (`.github/workflows/refresh-references.yml`) runs every quarter (Jan / Apr / Jul / Oct 1 at 17:00 UTC) plus on `workflow_dispatch`, with a `target` selector covering `all` / `federal` / `wa` / `or` / `ca` / `co` / `in` / `ny`. Each leg refreshes its state's content corpora via the matching pull scripts above, lints, and opens a PR auto-assigned for review. Federal content refreshes once per quarter (single canonical copy in the shared plugin) instead of per state. **New York is fully wired in**: `pull_ny_court_rules.py` fetches 22 NYCRR Parts 130/202/202.70/208/210/212/214/1200 + Tanbook + NYC Civil Court directives + Nassau / Suffolk DC local rules from `www.nycourts.gov` (falls back to pointer stubs when Cloudflare intercepts), and `pull_ny_statutes.py` hits the NY State Senate Open Legislation API at `legislation.nysenate.gov` (requires the `NYSENATE_API_KEY` repo secret to produce verbatim text — without the secret it writes well-formed pointer stubs covering the same 31 NY consolidated-laws targets, mirroring the Indiana puller's conditional-API pattern).
