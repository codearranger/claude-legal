#!/usr/bin/env python3
"""
Format checker for Arkansas court documents.

Arkansas has NO single statewide page/margin/font rule for trial-court
pleadings: the form of a pleading is governed by Ark. R. Civ. P. 10
(caption + numbered paragraphs + written instruments) and Rule 11
(signature), and typography/page limits come from each court's LOCAL
RULES / circuit administrative plan. This checker therefore validates
the marketplace COMMON-PRACTICE DEFAULTS and the marketplace-universal
layout conventions, and reports departures as WARN (not FAIL) where the
item is a local-rule matter rather than a statewide mandate.
...
reports against:
  ...
  - Line spacing >= 1.5 (double-spaced body is the common-practice default)
  - Body font size >= 12pt (24 half-points)
  - Black ink in the body (common-practice default)
  - No highlighting
  - Footer contains PAGE and NUMPAGES fields ("Page X of Y")
...
NOT LEGAL ADVICE. A clean report verifies surface formatting only.
Confirm the controlling court's local rules before filing.
"""
...
def check_font_family(root: ET.Element) -> CheckResult:
    ...
    acceptable = {
        ...
        "Book Antiqua",
        "Century Schoolbook",
        "Arial",  # Arial is also widely accepted in Arkansas practice
    }
    ...
    return CheckResult(
        "Font family",
        "WARN",
        f"Non-standard fonts detected: {sorted(fonts)}. Common Arkansas practice prefers a serif (Times New Roman) or Arial; confirm local rules.",
    )


def check_no_color(root: ET.Element) -> CheckResult:
    """
    Common-practice default: black ink in the body. Check for
    non-black text color and any highlighting.
    """
    violations: list[str] = []
    for color in root.findall(f".//{qn('w:color')}"):
        val = color.get(qn("w:val"), "auto")
        # Allow black, auto, and blue-black approximations
    ...
    if len(unique) <= 5:
        return CheckResult(
            "No color markings",
            "WARN",
            f"Color markings found: {unique}. Review — color is customarily reserved for exhibits that require it.",
        )
    return CheckResult(
        "No color markings",
        "FAIL",
        f"{len(violations)} color markings found. Body text is customarily black ink; color is reserved for exhibits that require it.",
    )
...
def format_report(path: Path, results: list[CheckResult]) -> str:
    ...
    lines = [f"Arkansas Format Check (Ark. R. Civ. P. 10/11 + common-practice defaults): {path.name}", "=" * 60]
    for r in results:
