#!/usr/bin/env python3
"""
22 NYCRR § 202.5 + § 202.5-b format checker for New York court documents.

Unpacks a .docx file, parses the document.xml, and reports
compliance with 22 NYCRR § 202.5's structural requirements:

  - Letter paper (8.5" x 11" / 12240 x 15840 DXA)
  - Margins >= 1" on all sides (1440 DXA)
  - Line spacing — double-spaced (>= 2.0x) for body
  - Body font size >= 12pt (24 half-points) — Commercial
    Division Rule 17 explicitly; default elsewhere
  - Black text on white (no highlighting / shading) — NYSCEF
    PDFs may include color in true exhibits, but body text
    should remain black
  - Footer contains PAGE field for pagination
...
def check_line_spacing(root: ET.Element) -> CheckResult:
    ...
    # 22 NYCRR § 202.5 requires double-spaced body (>= 480 in 240ths = 2.0x).
    # Quotations of 50+ words may be single-spaced and indented.
    if mode >= 480:
        if min_value >= 480:
            return CheckResult(
                "Line spacing",
        ...
        return CheckResult(
            "Line spacing",
            "PASS",
            f"Body spacing {mode_ratio:.2f}x (some footer/caption/blockquote paragraphs at {min_ratio:.2f}x — expected)",
        )
    return CheckResult(
        "Line spacing",
        "FAIL",
        f"Body mode spacing {mode_ratio:.2f}x is below 2.0x (22 NYCRR § 202.5 requires double-spaced body)",
    )
...
def check_font_family(root: ET.Element) -> CheckResult:
    ...
    acceptable = {
        ...
        "Book Antiqua",
        "Century Schoolbook",
        "Courier New",  # NY also accepts Courier
        "Arial",
    }
    ...
    return CheckResult(
        "Font family",
        "WARN",
        f"Non-standard fonts detected: {sorted(fonts)}. 22 NYCRR § 202.5 + Commercial Division Rule 17 prefer Times New Roman 12-pt; Courier and Arial are acceptable alternatives.",
    )


def check_no_color(root: ET.Element) -> CheckResult:
    """
    22 NYCRR § 202.5 requires legible black-text submissions on
    white paper. NYSCEF accepts color PDFs but body text should
    remain black — color is appropriate only for true exhibits
    (e.g., photographs) where it adds information.
    """
    violations: list[str] = []
    ...
    if len(unique) <= 5:
        return CheckResult(
            "No color markings",
            "WARN",
            f"Color markings found: {unique}. Review — 22 NYCRR § 202.5 expects legible black body text; color is appropriate only for true exhibits.",
        )
    return CheckResult(
        "No color markings",
        "FAIL",
        f"{len(violations)} color markings found. 22 NYCRR § 202.5 expects body text in black; color is acceptable only for true exhibits.",
    )


def check_footer_pagination(zf: zipfile.ZipFile) -> CheckResult:
    """
    Verify every generated document has pagination in the footer:
      - PAGE field (current page number)
      - Optionally NUMPAGES (for "of Y" — recommended but not
        strictly required by 22 NYCRR § 202.5; many NY Justices'
        Part Rules require "Page X of Y" format)
    """
    ...
    if not has_page:
        return CheckResult(
            "Footer pagination",
            "FAIL",
            "No PAGE field found in any footer — 22 NYCRR § 202.5 requires pagination",
        )
    if not has_numpages:
        return CheckResult(
            "Footer pagination",
            "WARN",
            "Footer has PAGE but no NUMPAGES — many NY Part Rules require 'Page X of Y' format; consider adding NUMPAGES",
        )
    return CheckResult(
        "Footer pagination",
        "PASS",
        "Footer has 'Page X of Y' pagination",
    )
...
def format_report(path: Path, results: list[CheckResult]) -> str:
    ...
    lines = [f"22 NYCRR § 202.5 Format Check: {path.name}", "=" * 60]
    for r in results:
