...
California's caption format differs fundamentally from Oregon's
and Washington's. It uses a **one-column layout** rather than a
side-by-side split:
...
(not "v." as in Oregon / federal court). Some California courts
use "v." but "vs." is conventional and accepted.
...
California pleadings require consecutively numbered lines on the
left margin. This is unique to California and does not appear in
Oregon, Washington, or federal courts (except for some district
courts' standing orders). Practical implementation:

- Pre-numbered pleading paper: Judicial Council paper with 28
  lines numbered 1-28 is widely used
- For programmatically generated documents: add a left column
  of numbers or use Word's line-numbering feature (Format →
  Paragraphs → Line Numbers → Continuous)
- For `.docx` generation with `docx-js`: implement as a
  two-column table (narrow left column with line numbers;
  wide right column with body text) — see
...
etc.) in most courts — this contrasts with Oregon's numbered
convention. Some courts and practitioners use numbered exhibits;
confirm with local rules or practice in the specific department.
...
- Page-1 top margin: 2.5 inches (not 3 inches as in Oregon)
- Subsequent pages: 1 inch top margin — requires a Word section
  break between page 1 and page 2
- Line numbering: implement as a narrow left column in a
  borderless two-column table (numbers 1-28 in the left column;
