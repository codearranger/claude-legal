...
def check_font_family(root: ET.Element) -> CheckResult:
    ...
    acceptable = {
        ...
        "Book Antiqua",
        "Century Schoolbook",
        "Arial",  # UTCR 2.010 accepts Arial as well
    }
    ...
    return CheckResult(
        "Font family",
        "WARN",
        f"Non-standard fonts detected: {sorted(fonts)}. UTCR 2.010 prefers a serif (Times New Roman) or Arial.",
    )


def check_no_color(root: ET.Element) -> CheckResult:
    """
    UTCR 2.010: 'black or blue-black ink only'. Check for non-black
    ...
    if len(unique) <= 5:
        return CheckResult(
            "No color markings",
            "WARN",
            f"Color markings found: {unique}. Review — UTCR 2.010 permits color only when grayscale would make exhibits illegible.",
        )
    return CheckResult(
        "No color markings",
        "FAIL",
        f"{len(violations)} color markings found. UTCR 2.010 requires black or blue-black ink only in submissions; color permitted only for exhibits that require it.",
    )
...
def format_report(path: Path, results: list[CheckResult]) -> str:
    ...
    lines = [f"UTCR 2.010 Format Check: {path.name}", "=" * 60]
    for r in results:
