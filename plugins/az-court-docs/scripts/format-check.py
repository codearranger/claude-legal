#!/usr/bin/env python3
"""
Ariz. R. Civ. P. 10 / 7.1 format checker for Arizona court documents.

Unpacks a .docx file, parses the document.xml and styles.xml, and
reports compliance with the Ariz. R. Civ. P. 10 / 7.1 structural
requirements:
  ...
  - No color text in the body (black ink only)
  - No highlighting
...
def check_font_family(root: ET.Element) -> CheckResult:
    ...
    acceptable = {
        ...
        "Book Antiqua",
        "Century Schoolbook",
        "Arial",  # Ariz. R. Civ. P. 7.1 accepts Arial as well
    }
    ...
    return CheckResult(
        "Font family",
        "WARN",
        f"Non-standard fonts detected: {sorted(fonts)}. Ariz. R. Civ. P. 7.1 prefers a serif (Times New Roman) or Arial.",
    )


def check_no_color(root: ET.Element) -> CheckResult:
    """
    Ariz. R. Civ. P. 7.1: black ink only. Check for non-black text
    color and any highlighting.
    """
    violations: list[str] = []
    ...
    if len(unique) <= 5:
        return CheckResult(
            "No color markings",
            "WARN",
            f"Color markings found: {unique}. Review — Ariz. R. Civ. P. 7.1 permits color only when grayscale would make exhibits illegible.",
        )
    return CheckResult(
        "No color markings",
        "FAIL",
        f"{len(violations)} color markings found. Ariz. R. Civ. P. 7.1 requires black ink only in submissions; color permitted only for exhibits that require it.",
    )
...
def format_report(path: Path, results: list[CheckResult]) -> str:
    ...
    lines = [f"Ariz. R. Civ. P. 10 / 7.1 Format Check: {path.name}", "=" * 60]
    for r in results:
