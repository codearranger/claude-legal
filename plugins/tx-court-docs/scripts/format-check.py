#!/usr/bin/env python3
"""
Texas court-document format checker.

Texas has no single statewide pleading-paper rule for the trial courts
(form flows from Tex. R. Civ. P. 45/47/57 and the Supreme Court's
e-filing technology standards under Tex. R. Jud. Admin. 10; the
appellate format spec is Tex. R. App. P. 9). This checker enforces the
marketplace's clean, court-safe baseline that produces a compliant
filing in any Texas court:

  - Letter paper (8.5" x 11" / 12240 x 15840 DXA)
  - Margins >= 1" on all sides (1440 DXA)
  - Line spacing >= 1.5
  - Body font size >= 12pt (24 half-points)
  - Black ink only (no color text in the body)
  - No highlighting
...
def check_font_family(root: ET.Element) -> CheckResult:
    ...
    return CheckResult(
        "Font family",
        "WARN",
        f"Non-standard fonts detected: {sorted(fonts)}. A legible serif (Times New Roman) or Arial is customary in Texas filings.",
    )


def check_no_color(root: ET.Element) -> CheckResult:
    """
    Texas filings use black ink. Check for non-black
    ...
    if len(unique) <= 5:
        return CheckResult(
            "No color markings",
            "WARN",
            f"Color markings found: {unique}. Reserve color for exhibits that require it; the body should be black ink.",
        )
    return CheckResult(
        "No color markings",
        "FAIL",
        f"{len(violations)} color markings found. Texas filings use black ink in the body; color permitted only for exhibits that require it.",
    )
...
def format_report(path: Path, results: list[CheckResult]) -> str:
    ...
    lines = [f"Texas Format Check: {path.name}", "=" * 60]
    for r in results:
