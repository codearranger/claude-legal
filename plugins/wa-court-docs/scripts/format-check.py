...
def check_font_size(root: ET.Element) -> CheckResult:
    ...
    # Body size typically is the mode of all sizes
    from collections import Counter
...
def check_font_family(root: ET.Element) -> CheckResult:
    ...
    if used_acceptable:
        return CheckResult(
            "Font family",
            "PASS",
            f"Uses acceptable serif font(s): {sorted(used_acceptable)}",
        )
    return CheckResult(
        "Font family",
        "WARN",
        f"Non-standard fonts detected: {sorted(fonts)}. GR 14 prefers a proportional serif.",
    )


def check_no_color(root: ET.Element) -> CheckResult:
    """
    GR 14(a): 'No color markings'. Check for non-auto text color and
    any highlighting.
    """
    violations: list[str] = []
    ...
    if len(unique) <= 5:
        return CheckResult(
            "No color markings",
            "WARN",
            f"Color markings found: {unique}. Review — GR 14(a) permits color only when grayscale would make exhibits illegible.",
        )
    return CheckResult(
        "No color markings",
        "FAIL",
        f"{len(violations)} color markings found. GR 14(a) prohibits color in submissions except exhibits that require it.",
    )
...
def format_report(path: Path, results: list[CheckResult]) -> str:
    ...
    lines = [f"GR 14 Format Check: {path.name}", "=" * 60]
    for r in results:
