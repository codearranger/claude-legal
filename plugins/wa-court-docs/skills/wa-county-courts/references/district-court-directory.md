# Washington county-court directory (most populous counties)

Courthouse addresses, divisions, e-filing systems, and known
local-practice notes for the district courts (and superior court clerks,
where they differ) of Washington's most populous counties — **excluding
King County District Court, which is covered by the `wa-kcdc` skill.**

> Verify everything here against the court's own page and the Washington
> Courts directory (`https://www.courts.wa.gov/court_dir/`) before you put
> it in a caption or rely on it for a deadline. Addresses, division
> assignments, portals, and calendar days change without notice.

---

## Pierce County (Tacoma) — ≈930k

**Pierce County District Court**
- County page: `https://www.piercecountywa.gov/101/District-Court`
- Courthouse: County-City Building, 930 Tacoma Ave S, Tacoma, WA 98402.
- E-filing: **LINX** — `https://linxonline.co.pierce.wa.us/` (free LINX
  account required). E-filing and e-service are **mandatory** for most
  filers under **PCLGR 30 / PCLR 30**; read that rule before your first
  filing.
- Local rules: Pierce County District Court LCR — from
  `https://www.courts.wa.gov/court_rules/` follow "District Court Rules"
  to the court-by-court list, or use the county site. Check the current
  civil motion docket and confirmation requirements there. (The in-repo
  court-rules corpus has the statewide CR/CRLJ/etc., not Pierce's local
  rules.)
- Records / clerk contact: `PCDCRecords@piercecountywa.gov`.

**Pierce County Superior Court** (general jurisdiction)
- Clerk: Pierce County Clerk, County-City Building, 930 Tacoma Ave S,
  Tacoma, WA 98402. Uses LINX as well; working copies are generally
  required for superior court motions — confirm via PCLR.

---

## Snohomish County (Everett + branches) — ≈844k

**Snohomish County District Court** — four physical divisions; **file in
the division assigned to the case**:
- **Everett Division** — 3000 Rockefeller Ave, Everett, WA 98201.
- **South Division** — 20520 68th Ave W, Lynnwood, WA 98036.
- **Evergreen Division** — 14414 179th Ave SE, Monroe, WA 98272.
- **Cascade Division** — 415 E Burke Ave, Arlington, WA 98223.
- County page: `https://snohomishcountywa.gov/194/About-the-District-Court-District-Court-`
  (court divisions / locations: `https://snohomishcountywa.gov/524/Court-Divisions`).
- E-filing: Snohomish County e-filing portal — link from the county page.
- Local rules: Snohomish County District Court local rules — see
  `https://www.courts.wa.gov/court_rules/` and the county site for the current civil
  calendar and any confirmation requirement.

**Snohomish County Superior Court**
- Clerk: Snohomish County Clerk, 3000 Rockefeller Ave, Everett, WA 98201.
  Superior court e-filing via the Snohomish County portal; working copies
  generally required (see Snohomish County LCR).

---

## Spokane County (Spokane) — ≈549k

**Spokane County District Court**
- County page: `https://www.spokanecounty.gov/2776/About-the-Court`
  (locations/hours: `https://www.spokanecounty.gov/469/Court-Locations-Hours`).
- Courthouse: Public Safety Building, 1100 W Mallon Ave, Spokane, WA
  99260 (mailing: PO Box 2352, Spokane, WA 99210-2352).
- E-filing: Spokane County e-filing portal — link from the county page.
- Local rules: Spokane County District Court local rules at
  `https://www.courts.wa.gov/court_rules/`.

**Spokane County Superior Court**
- Clerk: Spokane County Clerk, 1116 W Broadway Ave, Spokane, WA 99260.

---

## Clark County (Vancouver) — ≈517k

**Clark County District Court**
- County page: `https://clark.wa.gov/district-court` (contact:
  `https://clark.wa.gov/district-court/contact-information`).
- Courthouse: Public Service Center, 1300 Franklin St, Suite 160,
  Vancouver, WA 98660 (mailing: PO Box 9806, Vancouver, WA 98666-8806).
- E-filing: Clark County e-filing portal (Odyssey-based) — link from the
  district court page.
- Local rules: Clark County District Court local rules at
  `https://www.courts.wa.gov/court_rules/`.

**Clark County Superior Court**
- Clerk: Clark County Clerk — `https://clark.wa.gov/clerk` (contact:
  `https://clark.wa.gov/clerk/contact-information`).

---

## Thurston County (Olympia) — ≈300k

**Thurston County District Court**
- County page: `https://www.thurstoncountywa.gov/departments/district-court`
  (contact: `.../welcome-district-court/contact-us`).
- Courthouse: Building 3, 2000 Lakeridge Dr SW, Olympia, WA 98502.
- E-filing: Thurston County e-filing portal — link from the county page.
- Local rules: Thurston County District Court local rules at
  `https://www.courts.wa.gov/court_rules/`.

**Thurston County Superior Court**
- Clerk: Thurston County Clerk, 2000 Lakeridge Dr SW, Olympia, WA 98502.

---

## Kitsap County (Port Orchard) — ≈280k

**Kitsap County District Court**
- County page: `https://www.kitsap.gov/dc/Pages/default.aspx`.
- Courthouse: 614 Division St, MS-25, Port Orchard, WA 98366.
- E-filing: Kitsap County e-filing portal — link from the county page.
- Local rules: Kitsap County District Court local rules at
  `https://www.courts.wa.gov/court_rules/`.

**Kitsap County Superior Court**
- Clerk: Kitsap County Clerk, 614 Division St, Port Orchard, WA 98366
  (`https://www.kitsap.gov/sc/Pages/default.aspx`).

---

## Yakima County (Yakima) — ≈258k

**Yakima County District Court** — two physical locations:
- **Yakima** — Yakima County Courthouse, 128 N 2nd St, Room 225, Yakima,
  WA 98901.
- **Grandview branch** — 1313 W Wine Country Rd, Grandview, WA 98930.
- County page: `https://www.yakimacounty.us/412/District-Court`
  (all courts: `https://www.yakimacounty.us/410/Yakima-County-Courts`).
- E-filing: Yakima County e-filing portal — link from the county page.
- Local rules: Yakima County District Court local rules at
  `https://www.courts.wa.gov/court_rules/`.

**Yakima County Superior Court**
- Clerk: Yakima County Clerk, 128 N 2nd St, Yakima, WA 98901
  (`https://www.yakimacounty.us/486/Superior-Court`).

---

## Whatcom County (Bellingham) — ≈232k

**Whatcom County District Court**
- County page: `https://www.whatcomcounty.us/420/District-Court`.
- Courthouse: 311 Grand Ave, Suite 401, Bellingham, WA 98225.
- E-filing: Whatcom County e-filing portal — link from the county page.
- Local rules: Whatcom County District Court local rules at
  `https://www.courts.wa.gov/court_rules/`.

**Whatcom County Superior Court**
- Clerk: Whatcom County Clerk, 311 Grand Ave, Bellingham, WA 98225.

---

## Benton County (Kennewick) — ≈213k

**Benton County District Court**
- County page: `https://www.bentoncountywa.gov/departments/courts_and_judicial_services/district_court/index.php`.
- Courthouse: 7122 W Okanogan Pl, Suite A110, Kennewick, WA 99336
  (civil fees can be paid via the county's NCourt portal).
- E-filing: Benton County e-filing portal — link from the county page.
- Local rules: Benton County District Court local rules at
  `https://www.courts.wa.gov/court_rules/`.

**Superior Court — Benton & Franklin Counties** (a single shared superior
court for the two counties)
- `https://www.bentoncountywa.gov/pview.aspx?id=55107`. Benton County
  Justice Center, 7122 W Okanogan Pl, Kennewick, WA 99336.

---

## When you need a court not listed here

This skill covers the most populous counties. For any other Washington
county, go straight to the Washington Courts directory
(`https://www.courts.wa.gov/court_dir/`) for the address and judges, and
to `https://www.courts.wa.gov/court_rules/` → "Superior / District / Municipal Court
Rules" for the court-by-court list of local rules (the in-repo
`wa-law-references/references/court-rules/` corpus has the statewide rule
sets, not the local ones).
The cross-county workflow in `references/filing-and-service.md` still
applies.
