...
patterns. Recognizing the pattern tells you which defenses are
strongest, which discovery to serve first, and where the plaintiff
is likely to fold.

> Use this catalog to **triage** a new case. For any match, the
> "strongest defenses" list orders tactics by expected leverage,
> not by order of operations. For procedural order, see
> `wa-first-30-days`.
