#!/usr/bin/env python3
"""
Georgia court-document format checker.

Georgia has no single statewide pleading-paper rule. Document form
...
def check_font_family(root: ET.Element) -> CheckResult:
    ...
    acceptable = {
        ...
        "Book Antiqua",
        "Century Schoolbook",
        "Arial",  # sans-serif also accepted
    }
    ...
    return CheckResult(
        "Font family",
        "WARN",
        f"Non-standard fonts detected: {sorted(fonts)}. Prefer a serif (Times New Roman) or Arial for Georgia filings.",
    )


def check_no_color(root: ET.Element) -> CheckResult:
    """
    Black ink only in the body. Check for non-black text color and
    any highlighting.
    """
    violations: list[str] = []
    ...
    if len(unique) <= 5:
        return CheckResult(
            "No color markings",
            "WARN",
            f"Color markings found: {unique}. Review — use color only when grayscale would make exhibits illegible.",
        )
    return CheckResult(
        "No color markings",
        "FAIL",
        f"{len(violations)} color markings found. Georgia filings should use black ink only in the body; color permitted only for exhibits that require it.",
    )
...
def format_report(path: Path, results: list[CheckResult]) -> str:
    ...
    lines = [f"Georgia Format Check: {path.name}", "=" * 60]
    for r in results:
