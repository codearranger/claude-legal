#!/usr/bin/env python3
"""
Colorado format checker for court documents (C.R.C.P. 10 + Chief
Justice Directive 11-01).

Unpacks a .docx file, parses document.xml and styles.xml, and reports
compliance with Colorado's structural pleading requirements:

  - US Letter paper (8.5" x 11" / 12240 x 15840 DXA)
  - Margins >= 1" on all sides (1440 DXA); see C.R.C.P. 10(d)
  - Line spacing >= 1.5 (CJD 11-01)
  - Body font size >= 12pt (24 half-points)
  - No color text in the body (CJD 11-01 — black ink only)
  - Footer contains page numbers (CJD 11-01 — page X of Y)
  - Caption present with case number and division/courtroom

Authority: C.R.C.P. 10 (Form of Pleadings, Motions, and Other
Documents) and Colorado Chief Justice Directive 11-01 (Standards for
the Filing of Pleadings in Colorado State Courts).
...
def check_paper_size(root: ET.Element) -> CheckResult:
    ...
    return CheckResult(
        "Paper size",
        "FAIL",
        f"Expected 12240 x 15840 DXA (Letter), got {w} x {h} — CJD 11-01",
    )
...
def check_margins(root: ET.Element) -> CheckResult:
    ...
    return CheckResult(
        "Margins",
        "FAIL",
        f"Margins below 1\" minimum (CJD 11-01): {in_small}",
    )


def check_line_spacing(root: ET.Element) -> CheckResult:
    """CJD 11-01: pleadings double-spaced; block quotes and footnotes
    may be single-spaced. Pass if mode (body) spacing >= 1.5x to leave
    room for the 1.5-spaced motions most divisions also accept."""
    ...
    return CheckResult(
        "Line spacing",
        "FAIL",
        f"Body mode spacing {mode_ratio:.2f}x is below 1.5x (CJD 11-01)",
    )


def check_font_size(root: ET.Element) -> CheckResult:
    """CJD 11-01: pleadings in 12-point font."""
    sizes: list[int] = []
    ...
    return CheckResult(
        "Font size",
        "FAIL",
        f"Body font below 12pt (mode {mode/2:.1f}pt) — CJD 11-01",
    )


def check_font_family(root: ET.Element) -> CheckResult:
    """CJD 11-01 requires a clear, legible font. Colorado accepts both
    serif and sans-serif (unlike Washington's GR 14, which prefers
    serif)."""
    fonts = set()
    ...
    return CheckResult(
        "Font family",
        "WARN",
        f"Non-standard fonts: {sorted(fonts)}. CJD 11-01 requires a legible font.",
    )


def check_no_color(root: ET.Element) -> CheckResult:
    """CJD 11-01: pleadings shall be in black ink. Color is permissible
    only in exhibits where grayscale would lose meaning."""
    violations: list[str] = []
    ...
    if len(unique) <= 5:
        return CheckResult(
            "No color markings",
            "WARN",
            f"Color markings: {unique}. CJD 11-01 requires black ink; review whether the marking is an exhibit.",
        )
    return CheckResult(
        "No color markings",
        "FAIL",
        f"{len(violations)} color markings — CJD 11-01 requires pleadings in black ink.",
    )


def check_footer_pagination(zf: zipfile.ZipFile) -> CheckResult:
    """CJD 11-01: pages shall be numbered consecutively. Common
    practice is 'Page X of Y'."""
    footer_parts = [n for n in zf.namelist() if n.startswith("word/footer") and n.endswith(".xml")]
    if not footer_parts:
        return CheckResult("Footer pagination", "FAIL", "No footer part found (CJD 11-01)")
    ...
    if not has_page:
        return CheckResult(
            "Footer pagination",
            "FAIL",
            "No PAGE field found — CJD 11-01 requires page numbers",
        )
    if not has_numpages:
        return CheckResult(
            "Footer pagination",
...
def check_caption_division(root: ET.Element) -> CheckResult:
    """C.R.C.P. 10(a) + CJD 11-01: the caption of a Colorado district
    court pleading must include court name, case number, AND the
    division/courtroom assignment. Heuristic check on first ~1500
    chars of body text."""
    all_text = ""
    for t in root.findall(f".//{qn('w:t')}"):
        if t.text:
    ...
    has_case_no = ("case number" in head) or ("case no" in head)
    has_div = ("division" in head) or ("ctrm" in head) or ("courtroom" in head)
    if has_case_no and has_div:
        return CheckResult(
            "Caption components",
            "PASS",
            "Caption includes case number and division/courtroom",
        )
    missing = []
    if not has_case_no:
        missing.append("case number")
    if not has_div:
        missing.append("division/courtroom")
    return CheckResult(
        "Caption components",
        "WARN",
        f"Could not detect: {missing}. C.R.C.P. 10(a) + CJD 11-01 require both.",
    )
...
def run_checks(docx_path: Path) -> list[CheckResult]:
    ...
    with zipfile.ZipFile(docx_path) as zf:
        ...
        results.append(check_caption_division(doc_root))
...
def format_report(path: Path, results: list[CheckResult]) -> str:
    ...
    lines = [
        f"C.R.C.P. 10 / CJD 11-01 Format Check: {path.name}",
        "=" * 60,
    ]
    for r in results:
