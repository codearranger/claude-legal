#!/usr/bin/env python3
"""
I.R.C.P. 2 / 10 format checker for Idaho court documents.

Unpacks a .docx file, parses the document.xml and styles.xml, and
reports compliance with the I.R.C.P. 2 form-of-documents requirements:

  - Letter paper (8.5" x 11" / 12240 x 15840 DXA)
  - Top/side margins >= 1.2" (1728 DXA); bottom margin >= 1" (1440 DXA)
  - Line spacing >= 1.5 (double or one-and-one-half)
  - Body font size >= 11pt (22 half-points)
  - Black ink only (no color text or highlighting)
  - Footer contains PAGE and NUMPAGES fields (document title at bottom
    of each page per I.R.C.P. 2)

I.R.C.P. 2 (not Rule 10) carries Idaho's statewide format spec; Rule 10
cross-references Rule 2 and adds the numbered-paragraph requirement.
I.R.C.P. 2 permits slightly smaller margins only where doing so lets a
document fit on a single page — those are reported as WARN, not FAIL.
...
MIN_MARGIN_SIDE_TOP_DXA = 1728  # 1.2" (I.R.C.P. 2 — top and sides)
MIN_MARGIN_BOTTOM_DXA = 1440    # 1.0" (I.R.C.P. 2 — bottom)
...
def check_margins(root: ET.Element) -> CheckResult:
    ...
    for k, v in sides.items():
        floor = MIN_MARGIN_BOTTOM_DXA if k == "bottom" else MIN_MARGIN_SIDE_TOP_DXA
        if v < floor:
            too_small[k] = v
    if not too_small:
        ...
        return CheckResult(
            "Margins", "PASS",
            f"Top/sides >= 1.2\", bottom >= 1\": {in_sides}",
        )
    in_small = {k: f"{v/DXA_PER_INCH:.2f}\"" for k, v in too_small.items()}
    return CheckResult(
        "Margins",
        "WARN",
        f"Margins below the I.R.C.P. 2 minimum (top/sides 1.2\", bottom 1\"): "
        f"{in_small} — permitted only to fit a document on a single page",
    )
...
def check_line_spacing(root: ET.Element) -> CheckResult:
    ...
    'auto' rule uses 240ths: 360 = 1.5x, 480 = 2x. I.R.C.P. 2 requires
    double or one-and-one-half spacing.

    Some paragraphs (footer, captions) are legitimately single-spaced.
    Check the MODE (most common) spacing for body paragraphs.
    """
    ...
    return CheckResult(
        "Line spacing",
        "FAIL",
        f"Body mode spacing {mode_ratio:.2f}x is below the 1.5x I.R.C.P. 2 minimum",
    )
...
def check_font_size(root: ET.Element) -> CheckResult:
    ...
    I.R.C.P. 2 requires not less than 11pt = 22 half-points.
    """
    sizes: list[int] = []
    ...
    if min_size >= 22:
        return CheckResult(
            "Font size",
            "PASS",
            f"All runs >= 11pt (min {min_size/2:.1f}pt; mode {mode/2:.1f}pt)",
        )
    if mode >= 22:
        return CheckResult(
            "Font size",
            "WARN",
            f"Some runs below 11pt (min {min_size/2:.1f}pt) but body mode is {mode/2:.1f}pt",
        )
    return CheckResult(
        "Font size",
        "FAIL",
        f"Body font appears below the 11pt I.R.C.P. 2 minimum (mode {mode/2:.1f}pt)",
    )
...
def check_font_family(root: ET.Element) -> CheckResult:
    ...
    return CheckResult(
        "Font family",
        "WARN",
        f"Non-standard fonts detected: {sorted(fonts)}. I.R.C.P. 2 does not name a font; a legible serif (Times New Roman) is customary.",
    )


def check_no_color(root: ET.Element) -> CheckResult:
    """
    I.R.C.P. 2: black ink only. Check for non-black text color and
    any highlighting.
    """
    violations: list[str] = []
    ...
    if len(unique) <= 5:
        return CheckResult(
            "No color markings",
            "WARN",
            f"Color markings found: {unique}. I.R.C.P. 2 calls for black ink; reserve color for exhibits that require it.",
        )
    return CheckResult(
        "No color markings",
        "FAIL",
        f"{len(violations)} color markings found. I.R.C.P. 2 requires black ink in submissions; color permitted only for exhibits that require it.",
    )
...
def check_footer_pagination(zf: zipfile.ZipFile) -> CheckResult:
    ...
    Verify every generated document has the required footer:
      ...
      - A document title on the left (non-pagination literal text) —
        I.R.C.P. 2 places the document title at the bottom of each page.
    """
...
def format_report(path: Path, results: list[CheckResult]) -> str:
    ...
    lines = [f"I.R.C.P. 2 / 10 Format Check: {path.name}", "=" * 60]
    for r in results:
